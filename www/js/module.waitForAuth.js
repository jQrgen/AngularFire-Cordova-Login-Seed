 ...
 */
   .service('waitForAuth', function($rootScope, $q, $timeout) { // DETTA ER EN PROMISE
      var def = $q.defer(), subs = []; //http://blog.mediumequalsmessage.com/promise-deferred-objects-in-javascript-pt1-theory-and-semantics
      subs.push($rootScope.$on('$firebaseSimpleLogin:login', fn));
      subs.push($rootScope.$on('$firebaseSimpleLogin:logout', fn));
      subs.push($rootScope.$on('$firebaseSimpleLogin:error', fn)); // lytter på firebasesimplelogin events.
      function fn(err) {
         if( $rootScope.auth ) {
